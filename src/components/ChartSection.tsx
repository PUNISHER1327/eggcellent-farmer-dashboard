  ...
  return (
    <section id="charts" className="py-20">
      <div className="container mx-auto px-4">
        <h2 className="text-3xl md:text-4xl font-bold mb-2 text-gradient">Farm Analytics</h2>
        <p className={`text-lg mb-10 ${theme === 'light' ? 'text-gray-700' : 'text-white/70'}`}>
          Detailed trend analysis and performance indicators
        </p>
              ...
              <ChartContainer config={chartConfig} className="h-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={timeSeriesData} margin={{ top: 5, right: 20, bottom: 25, left: 0 }}>
                    ...
                    />
                    <Line type="monotone" dataKey="temperature" stroke="var(--color-temp)" strokeWidth={2} name="temp" />
              ...
              <ChartContainer config={chartConfig} className="h-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={eggProductionData} margin={{ top: 5, right: 20, bottom: 25, left: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke={theme === 'light' ? '#e5e7eb' : '#374151'} />
                    <XAxis dataKey="day" stroke={theme === 'light' ? '#6b7280' : '#9ca3af'} />
                    <YAxis stroke={theme === 'light' ? '#6b7280' : '#9ca3af'} />
                    <ChartTooltip
                      content={
                        <ChartTooltipContent />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="glass-morphism p-4">
            ...
            <div className={`text-6xl font-bold text-center my-12 ${theme === 'light' ? 'text-gray-800' : 'text-white'}`}>86<span className="text-xl text-muted-foreground">/100</span></div>
